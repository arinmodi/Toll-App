...
public class BluPrintsPrinter {
    ...
    public BluPrintsPrinter(BluetoothSocket socket) {
        bluetoothSocket = socket;
    }
    public BluPrintsPrinter(Socket socket) {
        socket = socket;
    }
    ...
    public void POS_Font_bold_Left() throws IOException {
        byte[] ESC_font_bold = new byte[]{27, 33, 0};
        byte[] data = byteArraysToBytes(new byte[][]{ESC_font_bold});
        bluetoothSocket.getOutputStream().write(data, 0, data.length);
    }

    public void POS_Font_bold_Center() throws IOException {
        byte[] ESC_font_bold = new byte[]{27, 33, 1};
        byte[] data = byteArraysToBytes(new byte[][]{ESC_font_bold});
        bluetoothSocket.getOutputStream().write(data, 0, data.length);
    }
